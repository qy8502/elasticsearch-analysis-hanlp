...
## Fork后修改支持ES 7.14.0 + hanlp 1.8.2版本
...
```
Github：https://github.com/qy8502/elasticsearch-analysis-hanlp
 ```

### 这里需要按照你的es版本进行设置（以下以7.14.0为例）
**一定要选择版本一致的**

###设置gradle.properties文件
elasticsearchVersion = 7.14.0

将编译好的文件build\distributions\elasticsearch-analysis-hanlp-7.14.0.zip放置服务器build\distributions\elasticsearch-analysis-hanlp-7.14.0.zip


### 安装
需要放到es 安装目录 plugins 下面
```
[root@localhost software]# cd /usr/share/elasticsearch
[root@localhost elasticsearch]# /usr/share/elasticsearch/bin/elasticsearch-plugin install file:/usr/share/elasticsearch/elasticsearch-analysis-hanlp-7.14.0.zip  
```
...
analysis-hanlp
```
上面的命令显示我们的 analysis-hanlp 已经安装成功了。


### 下载数据文件
7.13以后验证严格，必须放到插件config目录，目录程序写死/etc/elasticsearch/analysis-hanlp，如果不一致需要修改程序重新编译。并设置可读权限。
``` 
[root@localhost elasticsearch]# cd /etc/elasticsearch/analysis-hanlp
[root@localhost analysis-hanlp]# wget http://download.hanlp.com/data-for-1.7.5.zip
[root@localhost analysis-hanlp]# unzip data-for-1.7.5.zip
[root@localhost analysis-hanlp]# chmod -R a+r  /etc/elasticsearch/analysis-hanlp/data
```
